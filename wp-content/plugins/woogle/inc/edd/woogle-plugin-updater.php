 ...
 * Forked from EDD_SL_Plugin_Updater v1.6 by Pippin Williamson
 *
 * @author Patterns In the Cloud<support@patternsinthecloud.com>
 * @version 1.0
 * @internal Copy and change the class name if used outside of this plugin
 */
class Woogle_Plugin_Updater {
	private $api_url = '';
	 ...
	 * @param string $_plugin_file
	 *        	Path to the plugin file.
	 * @param array $_api_data
	...
	function http_request_args($args, $url) {
		...
		if (strpos ( $url, 'https://' ) !== false && strpos ( $url, $this->api_url ) !== false /* strpos ( $url, 'edd_action=package_download' ) */
				&& strpos ( $url, 'license=' . $this->api_data ['license'] ) ) {
	...
	public function show_changelog() {
		...
		if (! current_user_can ( 'update_plugins' )) {
			wp_die ( __ ( 'You do not have permission to install plugin updates', 'edd' ), __ ( 'Error', 'edd' ), array (
					'response' => 403 
			) );
		}
		
		$response = $this->api_request ( 'plugin_latest_version', array (
				'slug' => $_REQUEST ['slug'] 
		) );
