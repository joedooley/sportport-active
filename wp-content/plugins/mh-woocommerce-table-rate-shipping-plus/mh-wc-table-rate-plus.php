<?php
/*
Copyright (c) 2014 mangohour.com / John Currie
		...
		if ( ! class_exists( 'MH_Table_Rate_Plus_Shipping_Method' ) ) {
			class MH_Table_Rate_Plus_Shipping_Method extends WC_Shipping_Method {
				...
				function init() {
					...
					$this->has_shipping_classes = (WC()->shipping->get_shipping_classes() ? true : false);
				...
				function version_check() {
					if ((get_option($this->first_run_option) != false) && function_exists('curl_version')) {
					
						$licence_key = $this->settings['licence_key'];
						$api_endpoint = 'http://mangohour.com/api';
						...
						$ch = curl_init();
						$url = "{$api_endpoint}/check?licence_key={$licence_key}&plugin_id=1&version={$version}&domain={$domain}";
						...
						/*
						if (!empty($result)) {
							$licence = json_decode($result, true);
							if ($licence['licence_status'] == 'invalid') {
								add_action('admin_notices', array($this, 'admin_message_invalid'), 10);
							}
						}
						*/
					}
