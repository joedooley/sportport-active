...
class wcva_PluginUpdateChecker_2_2 {
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		$this->purchase_url    = ''.site_url().'/wp-admin/admin.php?page=wc-settings&tab=products&section=display#woocommerce_wcva_purchase_code';
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = '<a href="https://codecanyon.net/item/wooswatches-woocommerce-color-or-image-variation-swatches/7444039?ref=phppoet">WooSwatches</a> plugin is up to date.';
			} else if ( $status == 'update_available' ) {
			
			    if (($this->is_code_valid) == "active") {
					$message = 'A new version of <a href="https://codecanyon.net/item/wooswatches-woocommerce-color-or-image-variation-swatches/7444039?ref=phppoet">WooSwatches</a> plugin is available';
				} else {
					$message= sprintf(
							'A new version of <a href="https://codecanyon.net/item/wooswatches-woocommerce-color-or-image-variation-swatches/7444039?ref=phppoet">WooSwatches</a> plugin is available. Please enter valid <a href="%s"><span> purchase code</span></a> to get the update.',
							$this->purchase_url
						);
				}
...
if ( !class_exists('Wcva_PucFactory', false) ):
 ...
 * versions by calling {@link Wcva_PucFactory::addVersion()}.
 *
 ...
 * want to instantiate one of them anyway, you can use {@link Wcva_PucFactory::getLatestClassVersion()}
 * to get the class name and then create it with <code>new $class(...)</code>.
 */
class Wcva_PucFactory {
...
Wcva_PucFactory::addVersion('PluginUpdateChecker', 'wcva_PluginUpdateChecker_2_2', '2.2');
Wcva_PucFactory::addVersion('PluginUpdate', 'wcva_PluginUpdate_2_2', '2.2');
Wcva_PucFactory::addVersion('PluginInfo', 'wcva_PluginInfo_2_2', '2.2');
Wcva_PucFactory::addVersion('PucGitHubChecker', 'PucGitHubChecker_2_2', '2.2');
