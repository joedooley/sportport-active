...
function feeds_main_table() {
    ...
    if(isset($_GET['tab']) == 'managefeed'){
        $where = 'WHERE f.type = "eBaySeller" ';
    }else{
       $where = 'WHERE f.type != "eBaySeller" ';
    }
    // Read the feeds
                ...
                foreach ( array_keys( $seq ) as $s ) {
                    ...
                    if ( $pendcount )
                        ...
                        <td><?php echo $this_feed['filename']; ?>
                           ...
                           <?php $upload_url =get_admin_url() . 'admin.php?page=eBay_settings_tabs&tab=managefeed&action=uploadFeed&id=' . $this_feed['id']; ?>
                            <span class="upload">
