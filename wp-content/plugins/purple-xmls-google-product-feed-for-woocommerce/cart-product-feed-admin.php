...
function cart_product_admin_menu() {
	...
	add_submenu_page(
		'cart-product-feed-admin',
		__( 'eBay', 'cart-product-strings' ),
		__('eBay',  'cart-product-strings'),
		'manage_options',
		'eBay_settings_tabs',
		'eBay_settings_tabs_action'
	);
}
...
/**
 * cart-product-feed-ebay-page action function
 * Connectio with ebay 
 * @return Users login and token 
 * @author 
 **/
function eBay_settings_tabs_action()
{

    global $message;
	require_once 'cart-product-wpincludes.php';
	require_once('eBay_settings_tabs.php');
	$pageObj = new SettingsPage();
	$pageObj->loadNavigationTab();
	

	//do_action('cpf_init_pageview_ebay_setting');
}
