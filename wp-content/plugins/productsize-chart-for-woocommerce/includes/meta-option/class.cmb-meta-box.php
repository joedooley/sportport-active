...
class CMB_Chart_Meta_Box {
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-scripts',plugin_dir_url( __FILE__ ) . 'js/cmb.js', array( 'jquery' ) );

		wp_localize_script( 'cmb-scripts', 'CMBData', array(
			'strings' => array(
				'confirmDeleteField' => __( 'Are you sure you want to delete this field?', 'productsize-chart-for-woocommerce' )
				)
			)
		);
	...
	function enqueue_styles() {
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.8', '>=' ) )
			wp_enqueue_style( 'cmb-styles', plugin_dir_url( __FILE__ ) . 'css/dist/cmb'.$suffix.'.css' );
		else
			wp_enqueue_style( 'cmb-styles', plugin_dir_url( __FILE__ ) . 'css/legacy.css' );
