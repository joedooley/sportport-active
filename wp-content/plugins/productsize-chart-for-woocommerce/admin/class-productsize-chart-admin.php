...
class productsize_chart_Admin {
	...
	$labels = array(
		...
		'menu_name'          => _x( 'Charts', 'admin menu', $this->plugin_name ),
		'name_admin_bar'     => _x( 'Chart', 'add new on admin bar', $this->plugin_name ),
		'add_new'            => _x( 'Add New', 'chart', $this->plugin_name ),
		...
		'all_items'          => __( 'All Charts', $this->plugin_name ),
	...
	public function productsize_chart_image_enqueue() {
     global $typenow;
        ...
        wp_register_script( 'meta-box-image', plugin_dir_url( __FILE__ ) . '/js/custom-script.js', array( 'jquery' ) );
	...
	public function  productsize_chart_menu(){
			$settings=add_submenu_page('woocommerce', 'NC Size Chart Settings', 'NC Size Chart Settings', 'manage_options', 'productsize_chart',
			array($this, 'productsize_chart_settings_form'));
	...
	public function productsize_chart_settings_page() {
			if ( $_POST["productsize_chart_submit"]) {
			...
			wp_redirect(admin_url('admin.php?page=productsize_chart&'.$param));
			exit;
			}
				}		
