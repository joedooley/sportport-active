...
if ( ! class_exists( 'WPSEO_Local_Metaboxes' ) ) {
	...
	class WPSEO_Local_Metaboxes {
		...
		function metabox_locations() {
			...
			echo '<br class="clear">';
			echo '<p>';
			echo '<label class="textinput" for="wpseo_business_location_logo">Location logo:</label>';
			...
			echo '<input id="wpseo_business_location_logo_button" class="wpseo_image_upload_button button" type="button" value="Afbeelding uploaden">';
			echo '<br class="clear">';
			...
			foreach ( $wpseo_local_core->days as $key => $day ) {
				...
				echo '<div class="clear opening-hour-second ' . ( ( false === $multiple_opening_hours ) ? 'hidden' : '' ) . '">';
			...
			<script>
				function WPSEO_InsertMap() {
					...
					if (location_id != 'all' && center_id != '') {
						alert("<?php _e( 'If you select a single location , please set center to \'all\'', 'yoast-local-seo' ); ?>");
						return;
					}
					<?php } ?>
					...
					window.send_to_editor("[wpseo_map " + id + term + center + " width=\"" + width + "\" height=\"" + height + "\" zoom=\"" + zoom + "\" map_style=\"" + map_style + "\"" + scrollable + draggable + show_route + show_state + show_country + show_url + show_email + "]");
				}
				function WPSEO_InsertAddress() {
					...
					window.send_to_editor("[" + shortcode_name + " " + id + term + hide_name + oneline + show_state + show_country + show_phone + show_phone_2 + show_fax + show_email + show_url + show_vat + show_tax + show_coc + show_logo + show_opening_hours + hide_closed + orderby + order + "]");
				}
				function WPSEO_InsertOpeningHours() {
					...
					window.send_to_editor("[wpseo_opening_hours " + id + hide_closed + "show_days=\"" + show_days + "\"" + "]");
				}
				<?php if ( wpseo_has_multiple_locations() ) { ?>
					...
					<div class="wrap">
						<div>
							...
							<div style="padding:15px 15px 0 15px;">
								...
								<label for="wpseo_sl_show_filter"><input type="checkbox" id="wpseo_sl_show_filter" /> <?php _e( 'Show filter to narrow down search results', 'yoast-local-seo' ); ?>
								</label><br>
